...
// Use runtime-constructed RegExp to avoid ES5 target compile error with unicode property escapes
const emojiPresentationRegex = new RegExp('\\p{Emoji_Presentation}', 'gu')
				...
				appendTransaction: (transactions, oldState, newState) => {
					...
					getChangedRanges(changes).forEach(({ newRange }) => {
						...
						findChildrenInRange(newState.doc, newRange, (node) => node.type.isText).forEach(
							({ node, pos }) => {
								if (!node.text) return
								;[...node.text.matchAll(emojiPresentationRegex)].forEach((match) => {
