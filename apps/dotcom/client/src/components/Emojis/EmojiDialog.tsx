...
const EmojiDialog = React.forwardRef<HTMLDivElement, EmojiDialogProps>(function EmojiDialog(
	...
	const onPickerLoadedRef = useRef(onPickerLoaded)
	onEmojiSelectRef.current = onEmojiSelect
	onClickOutsideRef.current = onClickOutside
	onPickerLoadedRef.current = onPickerLoaded
	...
	useEffect(() => {
		...
		const picker = new Picker({
			data: EmojiData,
			maxFrequentRows: 0,
			onEmojiSelect: (emoji: EmojiMartData) => onEmojiSelectRef.current(emoji),
		...
		onPickerLoadedRef.current(picker)

		return () => {
			editor.off('event', eventListener)
		}
	}, [editor, breakpoint, theme, ref])
